...
#define TREE_TYPE_SP "d"
	...
	private:
		...
		struct Node
		{
			...
			explicit Node(T &x) : x(x), sum(x), p(nullptr) {}
			Node(T &x, Node *p) : x(x), sum(x), p(p) {}
			~Node()
			{
				delete l;
				delete r;
...
AVL::T AVL::sum(T l, T r)
{
	...
	if(l_node->x != x) l_node = next_below();
	if(r_node->x != x) r_node = prev_below();
}
...
bool AVL::next(T x, T *res)
{
	...
	return next_rq(root, x, &res_node, res);
}
...
bool AVL::prev(T x, T *res)
{
	if(!root) return false;
	Node *res_node = nullptr;
	return prev_rq(root, x, &res_node, res);
}
